...
parser_transitions.py: Feed-Forward Neural Network for Dependency Parsing
...
def minibatch_parse(sentences, model, batch_size):
    ...
    last_batch_size = len(sentences) % batch_size
    n_full_batch = int(len(sentences) / batch_size)
    batch_number = 0
    ...
    while len(unfinished_parses) > 0:
        ...
        for pp, prediction in zip(mini_batch, predictions):
            pp.parse([prediction])
